...
export const bookingOptionGroupDefinitions = [
  { id: "services", label: "Services", key: "services", type: "service" },
  { id: "timelines", label: "Timelines", key: "timelines", type: "timeline" },
  { id: "budgets", label: "Budgets", key: "budgets", type: "budget" },
  {
    id: "contact-methods",
...
export function getBookingOptionGroups() {
  return bookingOptionGroupDefinitions.reduce((groups, group) => {
    return {
      ...groups,
      [group.key]: listPublicBookingOptionsByType(group.type),
    };
  }, {});
}

export function getBookingOptionGroupSections() {
  const groups = getBookingOptionGroups();

  return bookingOptionGroupDefinitions.map((group) => ({
    id: group.id,
    label: group.label,
